...
function CourseList() {
    ...
    return (
      <div className="mt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-[#13181F] rounded-lg shadow-md p-6 animate-pulse">
  ...
  return (
    <div className='mt-6'>
      ...
      <div className='grid grid-cols-1 md:w-full md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {courses.map((course) => {
            ...
            <div key={course.id} className='bg-[#13181F] rounded-lg shadow-md p-6 border border-gray-800 hover:border-green-500 transition-all duration-300'>
              ...
              <div className='flex justify-between items-start mb-4'>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
                    <User size={14} />
                    <span>By {course.teacher_name || 'No teacher assigned'}</span>
                  </div>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs border ${statusBadge.color} flex items-center gap-1`}>
              ...
              {isEnrolled && (
                <div className='mb-4'>
                  <div className='flex justify-between items-center mb-2'>
                    ...
                    <span className='text-sm text-green-400 font-medium'>{course.progress}%</span>
                  </div>
              ...
              <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
                <div className="flex items-center gap-1">
                  <Clock size={12} />
                  <span>Last access: {formatDate(course.last_accessed)}</span>
                </div>
              ...
              <div className="mt-4">
                {isEnrolled ? (
                  <Link 
    href={`/workspace/my-courses/${course.id}`} // Always use numeric ID
    onClick={() => updateLastAccessed(course.enrollment_id, course.id)}
  >
    <Button className="w-full bg-green-600 hover:bg-green-500 transition-colors">
      <Play className='w-4 h-4 mr-2'/>
      Continue Learning
    </Button>
  </Link>
                ) : isPending ? (
                  <Button disabled className="w-full bg-yellow-600 hover:bg-yellow-600 cursor-not-allowed opacity-75">
